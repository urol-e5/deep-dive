...
GO_BP_plot_pi <- ggplot(shared_go_pi_df_bp, aes(x = Term, y = -log10(Fisher), shape = Species, fill = Species)) +
  ...
  theme(
    ...
    legend.text = element_text(size = 42),                      # Legend text size
...
ggsave(filename = "../figures/lncRNA_shared_GO_BP_plot.png", GO_BP_plot_lnc, width = 25, height = 35)
ggsave(filename = "../figures/lncRNA_shared_GO_BP_plot.pdf", GO_BP_plot_lnc, width = 25, height = 35)
```
...
They all share 1 overlap per 2 species and 4 overlaps for all species. what are the percentages for the venn diagram?
...
venn_mi <- ggvenn(
  ...
  set_name_size = 0,                    # Tries to hide set labels
  show_percentage = T,               # Show only percentages
  show_elements = F,
  stroke_alpha = 2,                     # Circle opacity
  text_size = 20                        # Adjust the percentage text size
); venn_mi
...
plot <- ggplot(species_data, aes(x = x, y = y, fill = Species)) +
  ...
  theme(legend.position = "right", 
         ...
         legend.text = element_text(size = 80),); plot  
...
```{r}
plot <- plot_grid(length_plot_lnc, venn_mi, length_plot_mi, venn_mi, length_plot_pi, grid.arrange(legend), labels = c("A", "B", "C", "D", "E"), label_size = 80, nrow = 3, ncol = 2)

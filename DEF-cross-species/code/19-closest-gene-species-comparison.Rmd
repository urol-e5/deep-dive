...
apul_go_lnc <- apul_go_lnc %>%
  dplyr::select(gene_name, GeneOntologyIDs, Term, Annotated, Significant, Expected, Fisher, ontology, prop.sig.genes) %>%
...
combo <- combo %>%
  ...
  dplyr::select(length,ACR,POC,POR) %>%
...
```{r}
apul_lnc_bed <- read.delim("../../D-Apul/output/26-Apul-lncRNA-expression-filtering/apul_final_lncRNAs.bed", header = F) %>%
...
peve_lnc_bed <- read.delim("../../E-Peve/output/23-Peve-lncRNA-expression-filtering/peve_final_lncRNAs.bed", header = F) %>%
  dplyr::select(V1, V2, V3)%>%
  mutate(length = V3-V2) %>%
  mutate(Species = "Porites evermanni")
...
pmea_lnc_bed <- read.delim("../../F-Pmea/output/23-Pmea-lncRNA-expression-filtering/pmea_final_lncRNAs.bed", header = F) %>%
  dplyr::select(V1, V2, V3)%>%
  mutate(length = V3-V2) %>%
  mutate(Species = "Pocillopora tuahiniensis")
...
length_plot_lnc <- ggplot(length_freq, aes(x = length_bin, y = frequency, fill = Species)) +
  ...
  theme(
    ...
    legend.text = element_text(size = 70, face = "italic"),
    legend.position = "top"                              
  ) +
  guides(fill = guide_legend(override.aes = list(size = 20)));  # Increase the size of legend squares
length_plot_lnc

...
GO_PEVE <- read.delim("../../data/Peve-rna-GO.tsv", sep = "\t", header = T)  %>%
  dplyr::select(query, GeneOntologyIDs) %>%
...
```{r}
pi_bed <- read.delim("../output/19-Peve-bedtools-closest/Peve_piRNA_output.bed", header = F)
head(pi_bed)
```
...
GO_PEVE_BP_En_sig_pi<-GO_PEVE_BP_En_pi[GO_PEVE_BP_En_pi$Fisher<0.05,]
```
...
# Write to csv 
write.csv(merged_pirna, "../output/19-Peve-bedtools-closest/Peve_GO_en_sig_gene_piRNA.csv")
```
...
```{r}
lnc_bed <- read.delim("../output/19-Peve-bedtools-closest/Peve_lncRNA_closest_mRNA_only.bed", header = F)
head(lnc_bed)
```
...
# Write to csv 
write.csv(merged_lncrna, "../output/19-Peve-bedtools-closest/Peve_GO_en_sig_gene_lncRNA.csv")
```

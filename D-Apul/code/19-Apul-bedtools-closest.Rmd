...
pi_bed_filt <- pi_bed_filt %>%
  ...
  dplyr::select(!gf_attr)
```

Calculate the overlap and distance based on the pi_start, pi_end, gf_start, and gf_end columns. Overlap occurs if the start of one feature is within the bounds of another. If there is no overlaps, it will be 0.
```{r}
pi_bed_filt$overlap <- pmax(
  0,
  pmin(pi_bed_filt$pi_end, pi_bed_filt$gf_end) -
    pmax(pi_bed_filt$pi_start, pi_bed_filt$gf_start) + 1)
```

Add length of features 
```{r}
pi_bed_filt <- pi_bed_filt %>%
  mutate(pi_length = pi_end - pi_start) %>%
  mutate(gf_length = gf_end - gf_start)
```
...
summary_stats <- pi_bed_filt %>%
  summarize(
    avg_overlap = mean(overlap_bp[overlap_bp > 0], na.rm = TRUE),
    min_overlap = min(overlap_bp[overlap_bp > 0], na.rm = T),
    max_overlap = max(overlap_bp[overlap_bp > 0], na.rm = T),
    avg_pi = mean(pi_length), na.rm = TRUE),
    min_pi = min(pi_length, na.rm = T),
    max_pi = max(pi_length, na.rm = T),
    median_overlap = median(overlap_bp[overlap_bp > 0], na.rm = TRUE),
    avg_distance = mean(overlap_bp[overlap_bp < 0], na.rm = TRUE),
    median_distance = median(overlap_bp[overlap_bp < 0], na.rm = TRUE),
    percent_overlap = sum(overlap_bp > 0) / n() * 100,
    percent_distance_lt_1kb = sum(overlap_bp < 0 & overlap_bp > -1000) / n() * 100,
    percent_distance = sum(overlap_bp < 0) / n() * 100)
...
# Write to csv 
write.csv(merged_pirna, "../output/19-Apul-bedtools-closest/Apul_GO_en_sig_gene_piRNA.csv")
```
...
```{r}
lnc_bed <- read.delim("../output/19-Apul-bedtools-closest/Apul_lncRNA_closest_mRNA_only.bed", header = F)
head(lnc_bed)
```
...
lnc_bed_filt <- lnc_bed %>%
  dplyr::select(V1, V2, V3, V4, V5, V7, V8, V9, V13) %>%
  dplyr::rename("l_chrom" = "V1", "l_start" = "V2", "l_end" = "V3", "gf_chrom" = "V5", "gf_type" = "V7", "gf_start" = "V8", "gf_end" = "V9", "gf_attr" = "V13")
```
...
```{r}
# Calculate distance
lnc_bed_filt$overlap <- pmax(0, 
    pmax(lnc_bed_filt$gf_start - lnc_bed_filt$l_end,  # mRNA is after lncRNA
         lnc_bed_filt$l_start - lnc_bed_filt$gf_end) # lncRNA is after mRNA
)

# Add a column to see if they overlap (TRUE/FALSE)
lnc_bed_filt$is_overlapping <- lnc_bed_filt$overlap == 0
...
Calculate average/percentages of distance and overlap. 
```{r}
lnc_summary <- lnc_bed_filt %>%
  group_by(V4) %>% # V4 is your unique lncRNA ID
  summarise(
    min_dist = min(distance),
    max_overlap = max(overlap),
    is_coding_neighbor = any(is_overlapping)
  )

# Calculate percentages on the summary
final_perc_overlap <- mean(lnc_summary$is_coding_neighbor) * 100
final_avg_dist <- mean(lnc_summary$min_dist[lnc_summary$min_dist > 0])
```
...
# Write to csv 
write.csv(merged_lncrna, "../output/19-Apul-bedtools-closest/Apul_GO_en_sig_gene_lncRNA.csv")
```

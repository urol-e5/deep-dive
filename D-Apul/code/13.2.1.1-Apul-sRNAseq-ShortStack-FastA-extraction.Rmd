...
awk '
# Function to increment the starting coordinate by 1
function increment_start(start) {
    return start + 1
}

# Process description lines starting with ">"
/^>/ {
    # Split the line into the main parts based on "::" delimiter
    split($0, main_parts, "::")
    
    # Split the coordinate part by colon and dash to separate out the start and end coordinates
    split(main_parts[2], coord_parts, "[:-]")
    
    # Increment the starting coordinate by 1
    new_start = increment_start(coord_parts[2])
    
    # Reconstruct the description line with the new starting coordinate
    new_description = main_parts[1] "::" coord_parts[1] ":" new_start "-" coord_parts[3] coord_parts[4]
...
# If FastA index doesn't exist, create it
if [ ! -f "${shortstack_dir}/${shortstack_fasta_index}" ]; then
  "${samtools} faidx" "${shortstack_dir}/${shortstack_fixed_fasta}"
fi

head "${shortstack_dir}/${shortstack_fixed_fasta}"
```
...
awk '$20 == "Y" && $21 != "NA" {print $2 ".mature" "::" $1 "(" $10 ")" }' "${shortstack_dir}/${shortstack_results_file}" \

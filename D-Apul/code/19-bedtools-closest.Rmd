...
```{r}
bed <- read.delim2("../output/19-bedtools-closest/miRNA_filtered_Apul_output_mRNA_only.bed", header = F)
head(bed)
```
...
```
Only has mRNAs, do not have to filter by genomic feature. Remove the siRNA m_types and make column that has cluster IDs for miRNAs. 
```{r}
bed_filt <- bed_filt %>%
...
60 miRNA features do not overlap with a transcript. 165 features do overlap with a transcript. Calculate average/percentages of distance and overlap for miRNA hairpins, mature miRNAs and miRNA stars.
...
length(unique(bed_filt_mature$m_ID))
```

Read in the mature named miRNAs 
```{r}
mature <- read.csv("../../DEF-cross-species/output/10-shortRNA-ShortStack-comparison/Apul_results_mature_named.csv", header = T)
head(mature)
```
...
head(bed_filt_mature_all)
```

Save output
```{r}
write.csv(file = "../output/19-bedtools-closest/Apul_mature_miRNAs_bedtools.csv", bed_filt_mature_all)
```

There are miRNAs matching up to multiple mRNAs but they don't actually seem to be unique mRNAs, as they have the same gf_start and gf_end but different gf_attr (ID= specifically). They appear to be from the same gene but different ID numbers. They may be variants...not sure. 
...
Read in GO annotation information. For Ptuh and Peve, I used the protein files but Steven also made an RNA annotation file for Apul. I initially tried to use the Apul protein file but the query IDs did not match. 
```{r}
annot <- read.delim("../output/20-Apul-gene-annotation/Apul-rna-GO.tsv")
```
...
bed_filt_mature_all$gf_ID <- gsub("ID=rna-", "", bed_filt_mature_all$gf_ID) 
```
...
# Make sure there are still 38 miRNAs
length(unique(bed_filt_mature_annot$m_ID))
```

Save output
```{r}
write.csv(file = "../output/19-bedtools-closest/Apul_mature_miRNAs_bedtools_annot.csv", bed_filt_mature_annot)
```

Look at the functions of the genes that overlap with miRNAs. 
```{r}

...
-r ../output/05.32-lncRNA-discovery/Apul_noncoding_transcripts_ids.txt > ../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt
...
```{r, engine='bash', eval=TRUE}
fgrep -c ">" ../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt
fgrep ">" ../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt | head -5

head ../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt
```

```{bash}
fgrep "468619	469943" ../output/05.32-lncRNA-discovery/Apul_lncRNA_candidates.gtf 
```



NC_058066.1	StringTie	transcript	468618	469943	.	+	.	transcript_id "MSTRG.40.1"; gene_id "MSTRG.40"; xloc "XLOC_000020"; class_code "u"; tss_id "TSS38";


```{r, engine='bash'}
#sed 's/transcript::\(.*\)/\1/' ../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt > ../output/05.32-lncRNA-discovery/patterns.txt
grep -F -f ../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt ../output/05.32-lncRNA-discovery/Apul_lncRNA_candidates.gtf > ../output/05.32-lncRNA-discovery/Apul_lncRNA.gtf
```


```{r, engine='bash'}
patterns_file="../output/05.32-lncRNA-discovery/Apul_lncRNAs.txt"
file_to_subset="../output/05.32-lncRNA-discovery/Apul_lncRNA_candidates.gtf"
output_file="../output/05.32-lncRNA-discovery/Apul_lncRNA_subset.gtf"

# Read each pattern and grep in the gtf file
while IFS= read -r line
do
    # Extract the pattern from the line
    pattern=$(echo "$line" | awk -F'::' '{print $2}' | sed 's/:/_/g; s/-/_/g')
    # Use grep to search the pattern in the gtf file and append matches to the output file
    grep "$pattern" "$file_to_subset" >> "$output_file"
done < "$patterns_file"
```
...
wc -l  ../output/05.32-lncRNA-discovery/Apul_lncRNA.gtf
head ../output/05.32-lncRNA-discovery/Apul_lncRNA.gtf
```

...
```{r}
pi_bed <- read.delim("../output/19-Pmea-bedtools-closest/Ptuh_piRNA_output.bed", header = F)
head(pi_bed)
```
...
# Write to csv 
write.csv(merged_pirna, "../output/19-Pmea-bedtools-closest/Pmea_GO_en_sig_gene_piRNA.csv")
```
...
```{r}
lnc_bed <- read.delim("../output/19-Pmea-bedtools-closest/Pmea_lncRNA_closest_mRNA_only.bed", header = F)
head(lnc_bed)
```
...
# Write to csv 
write.csv(merged_lncrna, "../output/19-Pmea-bedtools-closest/Pmea_GO_en_sig_gene_lncRNA.csv")
```

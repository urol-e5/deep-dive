...
L = list.files(path = "output/18-Pmea-piRNA/0_piRNA_pipeline_proTRAC/piRNA_length_data/", pattern=".txt", recursive = T)

names <- gsub("_piRNA_length.txt", "", L)

DFs = lapply(L, function(x) {
  data <- read.table(paste0("output/18-Pmea-piRNA/0_piRNA_pipeline_proTRAC/piRNA_length_data/", x), comment.char = "")
...
#save logo plot
my.pca.out <- paste0("output/18-Pmea-piRNA/3_length_and_nuc_dist/", my.outprefix,"piRNA_length_plot.pdf")
pdf(my.pca.out)
piRNA_length
dev.off()
...
sequences <- read.table("output/18-Pmea-piRNA/0_piRNA_pipeline_proTRAC/total_fasta_PMEA_replaced.fasta")

#make position weight matrix
pfm_PMEA <- consensusMatrix(sequences$V1)

#make logo plot
PMEA_plot <- ggseqlogo(pfm_PMEA, method = 'bits', seq_type = "rna") + ggtitle("Pocillopora meandrina")
...
my.logo.out <- paste0("output/18-Pmea-piRNA/3_length_and_nuc_dist/", my.outprefix,"PMEA_logo_plot.pdf")

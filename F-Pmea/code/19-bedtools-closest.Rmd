...
```{r}
bed <- read.delim("../output/19-bedtools-closest/miRNA_filtered_Ptuh_output.bed", header = F)
head(bed)
```
...
The Pmea genome only has the following genomic features annotated: CDS, exon, transcript. Subset by transcript, as the transcript encompasses both the CDS and exon regions in the genome. 
```{r}
bed_filt <- bed_filt %>%
...
39 miRNA features do not overlap with a transcript. 72 features do overlap with a transcript. Calculate average/percentages of distance and overlap for miRNA hairpins, mature miRNAs and miRNA stars.
```{r}
...
```{r}
mature <- read.csv("../../DEF-cross-species/output/10-shortRNA-ShortStack-comparison/Ptuh_results_mature_named.csv", header = T)
head(mature)
```
...
Save output
```{r}
write.csv(file = "../output/19-bedtools-closest/Ptuh_mature_miRNAs_bedtools.csv", bed_filt_mature_all)
```
...
```{r}
annot <- read.delim("../output/20-Pmea-gene-annotation/Pmea-protein-GO.tsv")
```

Remove "ID=" from `bed_filt_mature_all`
```{r}
bed_filt_mature_all$gf_attr <- gsub("ID=", "", bed_filt_mature_all$gf_attr)
```

Merge `annot` and `bed_filt_mature_all` dfs. 
```{r}
bed_filt_mature_annot <- bed_filt_mature_all %>%
  left_join(annot, by = c("gf_attr" = "query"))
```

Save output
```{r}
write.csv(file = "../output/19-bedtools-closest/Pmea_mature_miRNAs_bedtools_annot.csv", bed_filt_mature_annot)
```

There are 5 genes in the df that do not have annotations. Look at the functions of the genes that overlap with miRNAs. 
```{r}
...
Load Ptuh pi bed file into R. 
```{r}
bed <- read.delim("../output/19-bedtools-closest/Ptuh_piRNA_output.bed", header = F)
head(bed)
length(unique(bed$V1)) # 25 but should be 47
```

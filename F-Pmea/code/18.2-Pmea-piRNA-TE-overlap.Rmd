...
```{r}
genome_TEs <- bed_file %>%
  mutate(perc_g = ((V7-V6)/376580126)*100) %>%
...
cluster_bed <- read.table("output/18-Pmea-piRNA/2_TE_piRNA_cluster_overlap/PMEA_piRNA_cluster_TEs_intersect.txt")
cluster_bed$V7 <- gsub("/.*","", cluster_bed$V7)

cluster_TEs <- cluster_bed %>%
  mutate(perc_c = V9/512671*100) %>%
...
p1 <- ggplot(genome_TEs, aes(fill=V11, y=sum_perc_g, x=div_g)) + 
  ...
  scale_y_continuous(name = "Proportion of genome (%)") +
...
g <- ggplot_build(p1)
common_colors <- unique(g$data[[1]]["fill"])
common_colors <- as.character(common_colors$fill)
names(common_colors) <- c("DNA", "LINE","LTR","RC", "Retroposon","rRNA","Satellite","scRNA","SINE","snRNA","srpRNA","tRNA","Unknown")

p2 <- ggplot(cluster_TEs, aes(fill=V7, y=sum_perc_c, x=div_c)) + 
    geom_bar(position="stack", stat="identity") +
  scale_x_continuous(name = "Divergence from consensus (%)", breaks = c(0,5,10,15,20,25,30,35,40)) +
  scale_y_continuous(name = "Proportion of clusters (%)", breaks = c(0,.5,1,1.5,2,2.5,3)) +
  scale_fill_manual(values = common_colors) +
  theme_classic() +
  theme(legend.title = element_blank(), 
        legend.text = element_text(size = 7, margin = margin(r = 0, l = 0)),
...
coord_flip() +
  ...
  geom_text(aes(y=ifelse(cov_merge$n_fold <= 0, 0.5, -3.5), x = family,label=family, hjust = c(0,0.1,-0.2,0,0,0,0,0,0,0,0,0)), size = 3) 
fold_TE

TE_plot <- plot_grid(p1,p2, fold_TE, nrow = 1, align = "h", rel_widths = c(1,1,0.5), labels = c("D","E","F"))

ggsave(filename = paste0("output/18-Pmea-piRNA/2_TE_piRNA_cluster_overlap/", my.outprefix,"clusterTE_enrich_plot.png"), TE_plot, width = 12, height = 4)
